...
class Tanque():
	...
	def __init__(mismo, nivel, lado, posicion = None, direccion = None, filename = None):
		...
		# 0 - sin superpoderes
		# 1 - balas más rápidas
		# 2 - puede disparar 2 balas
		# 3 - puede destruir acero
		mismo.superPoderes = 0

		# cada tanque puede recoger 1 bono
		mismo.bonus = None
	...
	def fuego(mismo, forzado = False):
		...
		# si el nivel de superpoder es al menos 1
		if mismo.superPoderes > 0:
			bala.velocidad = 8

		# si el nivel de superpoder es al menos 3
		if mismo.superPoderes > 2:
			bala.poder = 2
		...
		if not fuego_amigo:
			...
			if mismo.vida < 1:
				if mismo.lado == mismo.LADO_ENEMIGO:
					...
					etiquetas.append(Etiqueta(mismo.rect.topleft, str(points), 500))
