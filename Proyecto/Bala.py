...
class Bala():
	...
	def actualizar(mismo, castillo):
		...
		if coliciones != []:
			for i in coliciones:
				if mismo.nivel.golpear_baldosa(castillo, rects[i].topleft, mismo.poder, mismo.owner == mismo.PROPIET_JUGAD):
					ha_colicionado = True
		if ha_colicionado:
			mismo.explotar()
			return
		...
		# verifica la colision con el castillo
		if castillo.active and mismo.rect.colliderect(castillo.rect):
			castillo.destruir()
			mismo.destruir()
			return
